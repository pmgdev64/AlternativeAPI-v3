package fr.trxyy.alternative.alternative_apiv3.minecraft.utils;
...
public enum OperatingSystem {
	...
	public static String getJavaPath(MinecraftVersion mcVersion, GameEngine engine) {
		String component = mcVersion.getJavaVersion().getComponent();
		GameFolder workDir = engine.getGameFolder();
		File javaPath = new File(workDir.getRuntimeDir(), component);
		if (getCurrentPlatform() == OperatingSystem.WINDOWS && new File(javaPath + "/bin/javaw.exe").isFile()) {
			return javaPath + "/bin/javaw.exe";
		}
