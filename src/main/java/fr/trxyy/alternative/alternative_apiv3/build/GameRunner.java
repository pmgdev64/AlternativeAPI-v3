package fr.trxyy.alternative.alternative_apiv3.build;
...
import fr.trxyy.alternative.alternative_apiv3.minecraft.json.Argument;
import fr.trxyy.alternative.alternative_apiv3.minecraft.json.ArgumentType;
...
public class GameRunner {
	...
	public GameRunner(Session ses, MinecraftVersion mcVersion, GameEngine engin) {
		...
		/** Ẩn Launcher khi chuẩn bị vào game */
		if (engine.getStage() != null) {
			Platform.runLater(new Runnable() {
				public void run() {
					engine.getStage().hide();
					// Gợi ý giải phóng RAM cho máy 4GB
					System.gc();
				}
			});
		}
    ...
    public void launch() throws Exception
    {
		...
		try {
			...
			/** Sử dụng luồng riêng để đợi game, tránh làm treo Launcher */
			Thread waitThread = new Thread(() -> {
				try {
					int exitVal = process.waitFor();
					if (exitVal != 0) {
						System.out.println("Minecraft has crashed (Exit code: " + exitVal + ")");
					} else {
						System.out.println("Minecraft closed normally.");
					}
					
					/** HIỆN LẠI LAUNCHER KHI GAME TẮT */
					Platform.runLater(() -> {
						if (engine.getStage() != null) {
			...
			waitThread.setName("Minecraft-Monitor");
			waitThread.start();
			
		} catch (IOException e) {
			throw new Exception("Cannot launch Minecraft!", e);
		}
	}
