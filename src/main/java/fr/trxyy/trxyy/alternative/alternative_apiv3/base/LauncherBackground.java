...
public class LauncherBackground {
	...
	public LauncherBackground(GameEngine engine, String mediaName, Pane root) {
	    this.posX = 0;
	    this.posY = 0;
	    String fileName = mediaName.toLowerCase();

	    // Nhánh xử lý VIDEO (MP4, AVI, MOV)
	    ...
	    // Nhánh xử lý IMAGE (Bao gồm cả GIF, PNG, JPG)
	    else {
	        ImageView backgroundImage = new ImageView();
	        // [Cải tiến]: Load trực tiếp qua javafx.scene.image.Image để hỗ trợ GIF tốt hơn
	...
	public Image loadImage(GameEngine engine, String image) {
	    try {
	        String fullPath = String.valueOf(engine.getLauncherPreferences().getResourceLocation()) + image;
	        URL url = ResourceLocation.class.getResource(fullPath);
	        
	        if (url == null) {
	            System.err.println("[AAuth] Không tìm thấy ảnh tại: " + fullPath);
	            return null;
	        }

	        // Dùng Constructor của JavaFX Image để hỗ trợ GIF động và load mượt hơn
	        // true: load ở background thread (giúp máy 4GB RAM không bị đơ giao diện)
